...
public:
	...
	//Lineal porque en el peor de los casos da k vueltas
	**/
	void cortaPorK(unsigned int k, Lista<T>& destino) {
		if (longitud() >= 1 && k <= longitud()) {
			...
			if (k == 1) {
				...
				destino._prim = p; //p = lk-1
				destino._numElems = longitud() - k;
				_ult = p->_ant;
				p->_ant->_sig = NULL; //DESENGANCHO
				p->_ant = NULL; //DESDENGANCHO
				_numElems = -k;
			}
	...
	//		if (k != longitud()) {
	//			p->_sig->_ant = NULL; //DESENGANCHO
	//			p->_sig = NULL; //DESDENGANCHO
	//		}
	//		else {
	//			_ult = NULL; // 1 2 3 4 5 5
	//		}
	//		_numElems -= k;
	//	}
	//}
