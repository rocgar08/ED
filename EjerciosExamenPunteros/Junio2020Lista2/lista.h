...
public:
	...
	**/
	void cortaPorK(unsigned int k, Lista<T>& destino) {
		// A IMPLEMENTAR
		if (k >= 1 && k <= longitud()) {
			//Necesito saber donde están los anteriores a K
			int pos = 1;
			Nodo* lK = _prim;
			Nodo* fin = _ult;
			//Busco la posicion donde está LK
			while (pos < k) {
				lK = lK->_sig;
				pos++;
			}
	...
	/* 5 1 2 3 4 5  lista origen 1 2 3 4  y la destino 5
	   5 1 2 3 4 5 */
	void imprime1() const {
		Nodo* n = _prim;
		while (n != NULL) {
