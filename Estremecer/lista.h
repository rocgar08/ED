...
public:
	...
	/*
	 ...
	 la operación
	 El coste es lineal, pòrque en el peor de los casos da n vueltas
	*/
	void estremece() {
		/* A IMPLEMENTAR */
		...
		if (longitud() > 2) {
			...
			while (cont < longitud()-1) {
				if (cont % 2 == 1) {
					...
					impar->_sig->_ant = impar->_ant; // 4 <-- 2
					impar->_ant->_sig = impar->_sig; // 2--> 4

					impar->_sig = fin->_sig; //fin de sig es 1, NULL al principo // 3 --> 1
					impar->_ant = fin; // 3 <---7
					fin->_sig = impar; //7 le digo que es el 1 //  7 --> 3
