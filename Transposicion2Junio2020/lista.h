...
public:
    ...
    /* 
	  ...
	  Lineal porque en el peor de los casos hay que recorrer la lista en caso de que el elemento minimo este al final.

	*/
	Nodo* min(Nodo*& a,Nodo*& b) {
	...
	void transponer() {
		// A IMPLEMENTAR
		Nodo* it = _ult;
		Nodo *mini = NULL, *minSig = NULL,*minAnt=NULL;
		bool encontrado = false;
		while (it != NULL && !encontrado) {

			if (it->_sig != NULL && it->_ant != NULL) {
				mini = min(it, it->_ant);
		...
		else if (mini!= _ult && mini!=_prim) {
			

			if (minAnt->_ant == NULL && minSig->_sig == NULL) {
				mini->_sig = NULL;
				_ult = mini;
			}
			else {
				if (minSig->_sig != NULL) minSig->_sig->_ant = mini;
				else {
					mini->_sig = NULL;
					_ult = mini;
				}
				mini->_sig = minSig->_sig;
