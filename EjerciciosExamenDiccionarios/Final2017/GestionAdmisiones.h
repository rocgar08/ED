...
class InfoPaciente {
public:
	InfoPaciente(const Paciente& paciente, const Lista<CodigoPaciente>::Iterator& posicion);
	const Paciente& datos() const;
	const Lista<CodigoPaciente>::Iterator& posicion() const;
private:
	Paciente _paciente;
...
private:
  ...
  // eficientemente este TAD
	DiccionarioHash<CodigoPaciente, InfoPaciente> _info; //Es hash  porque no me hace falta que sea ordenado
	Lista<CodigoPaciente> _lista_espera;// mi lista de espera
};
